...
public class MessageProcessor {
    ...
    MessageProcessor(MessageDispatcher messageDispatcher) {
        this.messageDispatcher = messageDispatcher;
    }

    record Task(PubsubMessage message, AckReplyConsumer consumer) {
    }

    public MessageReceiver gcpPubSubReceiver() {
        return (message, consumer) -> {
            try {
                messageQueue.put(new Task(message, consumer));
    ...
    public void start() {
        executorService.submit(() -> {
            while (!Thread.currentThread().isInterrupted()) {
                try {
                    Task task = messageQueue.take();
                    messageDispatcher.onMessage(task.message, task.consumer);
                } catch (InterruptedException e) {
                    Thread.currentThread().interrupt();
                    break;
                }
